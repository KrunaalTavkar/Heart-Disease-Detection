...
class KNN:
    ...
    def predict(self, features):
        ...
        ypred = []
        for i in Xtest:
            neighbors = KNN.get_k_neighbors(self, i)
            max_votes = Counter(neighbors)
            ypred.append(max_votes.most_common(1)[0][0])
    ...
    def get_k_neighbors(self, point):
        ...
        """
        distance_eu = []
        training_set = self.Xtrain
        dist_func = self.distance_function
        for x in training_set:
            d = dist_func(point, x)
            distance_eu.append(d)
        distance_eu_np = np.asarray(distance_eu)
        idx = np.argsort(distance_eu_np)
        neighbor_index = [y for y in idx[:self.k]]
        neighbor_labels = [self.ytrain[z] for z in neighbor_index]
